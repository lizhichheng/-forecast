package com.example.lastest.util;
...
public class Utility {
    ...
    public static TodayWeather handleWeatherResponse(String response) {
        try {
            JSONObject jsonObject = new JSONObject(response);
            JSONArray jsonArray = jsonObject.getJSONArray("HeWeather");
            String weatherContent = jsonArray.getJSONObject(0).toString();
            return new Gson().fromJson(weatherContent, TodayWeather.class);
        } catch (Exception e) {
            e.printStackTrace();
        }
        return null;
